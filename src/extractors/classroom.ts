...
export async function listCourses(
	...
	/** Titles/ids deliberately skipped as locked — DOM merge must not re-add them. */
	...
	const ingest = async () => {
		...
		for (let i = 0; i < allCourses.length; i++) {
			...
			if (skipLocked && !mapped.hasAccess) {
				lockedOut.add(mapped.id);
				lockedOut.add(mapped.title);
				if (mapped.nameHash) lockedOut.add(mapped.nameHash);
				continue;
			}
		...
		for (const c of fromDom) {
			...
			if (
				lockedOut.has(id) ||
				lockedOut.has(c.title) ||
				lockedOut.has(nameHash)
			) {
				continue;
			}
